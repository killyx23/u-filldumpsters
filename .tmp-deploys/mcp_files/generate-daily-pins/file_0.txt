...
async function generatePinWithFallback(accessToken, lockId, bridgeId, dropOffDate, dropOffTimeSlot, pickupDate, pickupTimeSlot, orderId) {
  ...
  const startDate = clampIgloohomeStart(buildIgloohomeDate(dropOffDate, dropOffTimeSlot, 12));
  // PIN stays valid 1 hour past scheduled return so late returns still open the lock
  const endDate = addGraceHour(buildIgloohomeDate(pickupDate, pickupTimeSlot, 5));
  ...
  try {
    ...
    for (const booking of eligibleBookings){
      ...
      try {
        ...
        const startTimeUTC = clampIgloohomeStart(buildIgloohomeDate(booking.drop_off_date, booking.drop_off_time_slot, 12));
        const endTimeUTC = addGraceHour(buildIgloohomeDate(booking.pickup_date, booking.pickup_time_slot, 5));

...
export function buildBookingLineItems({
  ...
  const addDisposal = (key, label, count, defaultId) => {
  ...
  addDisposal('mattressDisposal', 'Mattress Disposal', addonsData?.mattressDisposal, 4);
  addDisposal('tvDisposal', 'TV Disposal', addonsData?.tvDisposal, 5);
  addDisposal('applianceDisposal', 'Appliance Disposal', addonsData?.applianceDisposal, 6);
...
export function allocateDiscount(lineItems, discount) {
  ...
  const result = lineItems.map((line, index) => {
    let lineDiscount;

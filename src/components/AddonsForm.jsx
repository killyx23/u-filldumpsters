...
import { ArrowLeft, Check, Shield, HardHat, PackagePlus, ShoppingCart, Hammer, Minus, Plus, AlertTriangle, Loader2, MessageSquare, HelpCircle } from 'lucide-react';
...
export const AddonsForm = ({ basePrice, addonsData, setAddonsData, onSubmit, onBack, plan }) => {
  ...
  return (
    <>
          ...
          <div className="flex items-center justify-between mb-8">
            ...
            {plan.id !== 2 && (
              <div className="absolute top-4 right-4 flex items-center space-x-2">
                <Button onClick={onBack} variant="outline" className="text-white border-white/30 hover:bg-white/10 hover:text-white">
      ...
      <Dialog open={showInsuranceDeclineWarning} onOpenChange={setShowInsuranceDeclineWarning}>
        <DialogContent className="bg-gray-900 border-yellow-500 text-white">
          ...
          <DialogFooter className="gap-2 sm:justify-center">
            <Button onClick={() => setShowInsuranceDeclineWarning(false)} variant="outline" className="border-yellow-400 text-yellow-400 hover:bg-yellow-400 hover:text-black">Go Back & Accept</Button>
            <Button onClick={confirmDeclineInsurance} variant="destructive">I Understand & Decline</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>

      <Dialog open={showDrivewayDeclineWarning} onOpenChange={setShowDrivewayDeclineWarning}>
        <DialogContent className="bg-gray-900 border-yellow-500 text-white">
          <DialogHeader>
            <DialogTitle className="flex items-center text-yellow-400 text-2xl">
              <AlertTriangle className="mr-3 h-8 w-8" />
          ...
          <DialogDescription className="text-blue-200 my-4 text-base">
            By declining driveway protection, you acknowledge that U-Fill Dumpsters is not liable for any potential damage, including but not limited to cracks, scratches, or stains, to your driveway or the surrounding area.
          </DialogDescription>
          <DialogFooter className="gap-2 sm:justify-center">
            <Button onClick={() => setShowDrivewayDeclineWarning(false)} variant="outline" className="border-yellow-400 text-yellow-400 hover:bg-yellow-400 hover:text-black">Go Back & Accept</Button>
            <Button onClick={confirmDeclineDrivewayProtection} variant="destructive">I Understand & Decline</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </>

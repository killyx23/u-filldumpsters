...
export const ServiceDescriptionModal = ({ service, isOpen, onClose, referenceDate = null }) => {
    const {
        description,
        loading,
        features,
        ...
        mileageRate,
        planCard,
    } = useServiceScheduleDescription(service, referenceDate, isOpen);

    if (!service) return null;

    const serviceName = safeExtractString(planCard?.displayName || service?.name, 'Service Level');
    const priceUnit = safeExtractString(
        planCard?.displayPriceUnit || service?.price_unit,
        ''
    );
                    ...
                    <div className="space-y-6 pb-6">
                        <div className="space-y-2">
                            ...
                            <p className="text-gray-300 leading-relaxed">
                                {loading ? 'Loading schedule...' : description}
                            </p>
                        </div>
                        ...
                        <div className="space-y-3">
                            ...
                            <ul className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                {(features.length > 0 ? features : ['High quality equipment', 'Professional service', 'Flexible scheduling']).map((feature, idx) => {
                            ...
                            <ul className="space-y-2 text-sm text-gray-300">
                                ...
                                {deliveryFee > 0 && (
                                    <li className="flex justify-between border-b border-gray-800/50 pb-1">
                                        ...
                                        <span className="font-bold text-white">
                                            {formatCurrency(deliveryFee * 100, { code: 'USD', symbol: '$' })}
                                        </span>
                                    </li>
                                )}
                                {mileageRate > 0 && (
                                    <li className="flex justify-between border-b border-gray-800/50 pb-1">
                                        ...
                                        <span className="font-bold text-white">
                                            {formatCurrency(mileageRate * 100, { code: 'USD', symbol: '$' })}/mile
                                        </span>
                                    </li>
                                )}
                            </ul>

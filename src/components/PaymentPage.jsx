...
const CheckoutForm = ({
  ...
  const isDeliveryService = planName === "Dump Loader Trailer with Delivery" || planName === "16 Yard Dumpster Rental";
  ...
  const insuranceCharge = addonsData?.insurance === 'accept' ? insurancePrice : 0;
  ...
  const applianceCharge = (addonsData?.applianceDisposal || 0) * 35;
  ...
  const subtotal = basePriceAmount + deliveryFeeFlat + tripMileageCost + insuranceCharge + drivewayCharge + equipmentTotal + mattressCharge + tvCharge + applianceCharge;
  ...
  const estimatedTax = totalPrice > displayTotal + 0.01 ? totalPrice - displayTotal : 0;
  ...
  const canProceedWithPayment = isConfirmed && stripe && elements && isPaymentElementReady && (!isDeliveryService || delivery_location_verified);
      ...
      <div className="max-w-2xl mx-auto bg-slate-900/60 backdrop-blur-lg rounded-2xl p-8 shadow-2xl border border-white/20">
        ...
        <div className="bg-white/5 p-6 rounded-lg mb-8 border border-white/10">
          ...
          <div className="space-y-1 text-blue-100 font-mono text-sm">
            ...
            {deliveryFeeFlat > 0 && <BreakdownLine label="Delivery Fee (Flat)" value={deliveryFeeFlat} />}
            {tripMileageCost > 0 && <BreakdownLine label="Mileage Charge" value={tripMileageCost} />}
            {equipmentItems.map((item, idx) => <BreakdownLine key={idx} label={`${item.label}${item.quantity > 1 ? ` (x${item.quantity})` : ''}`} value={item.price} />)}
            ...
            <div className="border-t border-white/20 my-2 pt-2">
              ...
              {discountAmount > 0 && <BreakdownLine label={`Discount (${addonsData?.coupon?.code || 'Applied'})`} value={-discountAmount} />}
            ...
            <div className="border-t border-white/20 pt-3 mt-1">
              <div className="flex justify-between items-center text-white">
                ...
                <span className="text-2xl font-bold text-green-400">{formatMoney(displayTotal)}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <form onSubmit={handlePayment}>
          ...
          <Button type="submit" disabled={isProcessing || !canProceedWithPayment} className={`w-full py-6 text-xl font-bold transition-all duration-300 ${isProcessing || !canProceedWithPayment ? 'bg-white/10 text-white/50 cursor-not-allowed border border-white/10' : 'bg-gradient-to-r from-green-500 to-teal-600 hover:from-green-600 hover:to-teal-700 text-white shadow-xl shadow-green-900/40 border border-green-400/30 active:scale-[0.98]'}`}>
            {isProcessing ? <><Loader2 className="mr-3 h-6 w-6 animate-spin" />Processing...</> : <><CreditCard className="mr-3 h-6 w-6" />Pay {formatMoney(displayTotal)}</>}
          </Button>

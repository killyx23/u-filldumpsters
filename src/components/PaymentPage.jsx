...
export const PaymentPage = ({
  ...
  const subtotal = basePriceAmount + deliveryFee + insuranceCharge + drivewayCharge + equipmentTotal;
  ...
  const handlePayment = async (e) => {
    ...
    // stripe.confirmPayment handles everything:
    // card tokenization, 3DS, Apple Pay, Google Pay — all automatically
    // On success Stripe redirects to return_url, so code below only
    ...
    // Only reaches here if payment failed or was cancelled
    if (error) {
      console.error("[PaymentPage] confirmPayment error:", error);
      // Stripe error types: card_error and validation_error are user-facing
      // other types are unexpected and should show a generic message
    ...
    // If no error, Stripe is redirecting — don't setIsProcessing(false)
    // so the button stays in loading state during redirect
  };
    ...
    >
      <div className="max-w-2xl mx-auto bg-slate-900/60 backdrop-blur-lg rounded-2xl p-8 shadow-2xl border border-white/20">
        ...
        <form onSubmit={handlePayment}>
          {/* Payment Element — renders card, Apple Pay, Google Pay automatically */}
          <div className="mb-6">
            <PaymentElement
              onReady={() => setIsPaymentElementReady(true)}
              options={{
                layout: 'tabs', // shows tabs for card / Apple Pay / Google Pay
                fields: {
                  // We pass billing details manually in confirmParams above
                  // so tell Stripe not to render those fields in the form
                  billingDetails: {
                    name: 'never',
                    email: 'never',
                    phone: 'never',

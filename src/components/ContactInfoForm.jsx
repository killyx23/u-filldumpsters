...
export const ContactInfoForm = ({ bookingData, setBookingData, onSubmit, onBack }) => {
    ...
    const handleSubmit = (e) => {
        ...
        // Check if verified explicitly to prevent blocking valid addresses that just didn't use autocomplete
        if (!bookingData.contactAddress?.isVerified && !bookingData.addressVerified) {
            toast({ title: "Address Required", description: "Please ensure your contact address is verified.", variant: "destructive" });
            return;
        }
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        <div className="bg-black/20 p-6 rounded-xl border border-white/10 space-y-4">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <InputField icon={<User />} type="text" name="firstName" placeholder="First Name" value={bookingData.firstName} onChange={handleInputChange} required />
                                <InputField icon={<User />} type="text" name="lastName" placeholder="Last Name" value={bookingData.lastName} onChange={handleInputChange} required />
                            </div>
                        ...
                        <div className="bg-black/20 p-6 rounded-xl border border-white/10 space-y-4">
                            <div className="flex items-center justify-between mb-2">
                                ...
                                {(bookingData.contactAddress?.isVerified || bookingData.addressVerified) && (
                            ...
                            <div className="space-y-2">
                                <GooglePlacesAutocomplete 
                                    ...
                                    onChange={(val) => setBookingData(prev => ({
                                        ...prev, 
                                        addressVerified: false, // Task 2: Reset flag when manually changed
                                        contactAddress: {...prev.contactAddress, street: val, isVerified: false}
                                    }))} 
                                    onAddressSelect={(details) => setBookingData(prev => ({
                                        ...prev, 
                                        addressVerified: true, // Task 2: Explicitly set parent state verification flag
                                        contactAddress: {...details, isVerified: true}
                                    }))} 
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-2">
                                <InputField icon={<MapPin />} type="text" value={bookingData.contactAddress?.city || ''} placeholder="City" disabled />
                                <InputField icon={<MapPin />} type="text" value={bookingData.contactAddress?.state || ''} placeholder="State" disabled />
                                <InputField icon={<MapPin />} type="text" value={bookingData.contactAddress?.zip || ''} placeholder="ZIP Code" disabled />
                            </div>
                        </div>

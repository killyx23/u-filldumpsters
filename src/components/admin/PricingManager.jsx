...
import { Loader2, Save, Tag, Plus, Edit } from 'lucide-react';
...
const ServicePricingCard = ({ service, onSave }) => {
    const [price, setPrice] = useState(service.homepage_price !== null ? service.homepage_price : service.base_price);
    const [unit, setUnit] = useState(service.homepage_price_unit || service.price_unit);
    const [isSaving, setIsSaving] = useState(false);

    useEffect(() => {
        setPrice(service.homepage_price !== null ? service.homepage_price : service.base_price);
        setUnit(service.homepage_price_unit || service.price_unit);
    }, [service]);

    const handleSave = async () => {
        setIsSaving(true);
        await onSave(service.id, price, unit);
        setIsSaving(false);
    };
...
export const PricingManager = () => {
    ...
    const fetchData = useCallback(async () => {
        ...
        if (servicesRes.error) toast({ title: "Failed to load services", variant: "destructive" });
        else setServices(servicesRes.data);

        if (couponsRes.error) toast({ title: "Failed to load coupons", variant: "destructive" });
    ...
    const handleSaveService = async (id, price, unit) => {
        const { error } = await supabase
            .from('services')
            .update({ homepage_price: price, homepage_price_unit: unit })
            .eq('id', id);
        ...
        if (error) {
            ...
            fetchData(); // Refetch to ensure state is in sync with DB
        }
    };
    ...
    const handleToggleCouponStatus = async (coupon) => {
        const { error } = await supabase
            ...
            .eq('id', coupon.id);
        if (error) toast({ title: 'Failed to update coupon status', variant: 'destructive' });
        else {
    ...
    return (
        <div className="space-y-8">
            <div className="bg-white/10 p-6 rounded-2xl">
                <h2 className="text-2xl font-bold mb-4">Homepage Pricing</h2>

...
import { format, parseISO, isFuture, isToday, isYesterday, addHours, isSameDay, differenceInCalendarDays, addDays, isValid } from 'date-fns';
...
const BookingCard = ({ booking, onReceiptClick, onCancelClick, onRescheduleClick }) => {
    const { plan, drop_off_date, pickup_date, status, total_price, addons, reschedule_history, created_at, drop_off_time_slot, pickup_time_slot } = booking;
            ...
            <CardHeader>
                <div className="flex justify-between items-start">
                    <div>
                        ...
                        <CardDescription className="text-blue-200">ID: {booking.id}</CardDescription>
                    </div>
                    <div className="text-right">
                        <div className={`flex items-center justify-end gap-2 text-sm font-semibold ${statusInfo.color}`}>
                            {statusInfo.icon}
                            <span>{statusInfo.text}</span>
                        </div>
                        <CardDescription className="text-xs text-blue-300 mt-1">Booked: {format(parseISO(created_at), 'Pp')}</CardDescription>
                    </div>
                </div>
            </CardHeader>
            <CardContent className="space-y-2 text-sm">
                <p><strong>{plan?.id === 2 ? (isDelivery ? "Delivery" : "Pickup") : "Drop-off"}:</strong> {isPending ? 'Pending Review' : `${format(parseISO(drop_off_date), 'PPP')} - ${formatTime(drop_off_time_slot)}`}</p>
                {plan?.id !== 3 && <p><strong>{plan?.id === 2 ? "Return" : "Pickup"}:</strong> {isPending ? 'Pending Review' : `${format(parseISO(pickup_date), 'PPP')} - ${formatTime(pickup_time_slot)}`}</p>}
                <p><strong>Total:</strong> ${(total_price || 0).toFixed(2)}</p>
            </CardContent>
...
const Messenger = ({ customer, initialNotes, onNewNote }) => {
    ...
    const handleSendMessage = async (attachment = null) => {
        ...
        if (error) {
            ...
            onNewNote(data); // Update local state immediately
            setMessage('');
        }
        setIsSending(false);
    };
    ...
    const ChatBubble = ({ note }) => {
        ...
        const sourceText = isAdmin ? 'Scheduling' : 'You';
        ...
        <div className="flex flex-col h-[80vh] bg-gray-800 rounded-lg shadow-2xl">
            ...
            <div ref={chatContainerRef} className="flex-1 p-4 overflow-y-auto">
                {groupedNotes.map((group, index) => {
                    ...
                    return <ChatBubble key={group.note.id} note={group.note} />;
                })}
            </div>
...
export default function CustomerPortal() {
    ...
    const handleNewNote = (newNote) => {
        ...
        if (newNote.author_type === 'admin') {
            setHasUnread(true);
            toast({
                title: "New Message from Scheduling",
                description: newNote.content || "You have a new message.",
    ...
    const fetchData = useCallback(async (isInitialLoad = true) => {
        ...
        if (!customerDbId) {
             toast({ title: "Authentication Error", description: "Could not find your customer profile. Please log in again.", variant: "destructive" });
            if (isInitialLoad) setLoading(false);
            signOut();
            navigate('/login');
            return;
    ...
    useEffect(() => {
        ...
        const channel = supabase.channel(`customer-portal-realtime-${customerData.id}`)
            .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'customer_notes', filter: `customer_id=eq.${customerData.id}` }, (payload) => handleNewNote(payload.new))
            .on('postgres_changes', { event: '*', schema: 'public', table: 'bookings', filter: `customer_id=eq.${customerData.id}` }, () => fetchData(false))
            .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'customers', filter: `id=eq.${customerData.id}` }, () => fetchData(false))
            .subscribe();
    ...
    const sortedBookings = bookings ? [...bookings].sort((a, b) => new Date(b.created_at) - new Date(a.created_at)) : [];
        ...
        <div className="container mx-auto py-16 px-4">
            ...
            <Tabs defaultValue="bookings" className="w-full" onValueChange={handleTabChange}>
                ...
                <TabsContent value="messages">
                    <Messenger customer={customerData} initialNotes={notes} onNewNote={handleNewNote} />
                </TabsContent>

                <TabsContent value="calendar">
                    <MyCalendar bookings={sortedBookings} />
                </TabsContent>
            </Tabs>

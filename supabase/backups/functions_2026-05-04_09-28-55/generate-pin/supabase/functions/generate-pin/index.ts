...
function buildStartDate(dropOffDate) {
  ...
  if (dropOffDate === today) {
    // Same-day: use current UTC time + 5 min buffer
    const now = new Date(Date.now() + 5 * 60 * 1000);
    const datePart = now.toISOString().split("T")[0];
    return `${datePart}T${pad(now.getUTCHours())}:${pad(now.getUTCMinutes())}:00+00:00`;
  }
  // Future bookings: midnight MST = 06:00 UTC
  // Sending 00:00 UTC caused the lock to snap forward 6 hours to the next boundary
  return `${dropOffDate}T06:00:00+00:00`;
}
...
async function generatePinWithFallback(accessToken, lockId, bridgeId, dropOffDate, pickupDate, orderId) {
  ...
  const startDate = buildStartDate(dropOffDate);
  // 11:59 PM MST = 05:59 UTC next day
  // Sending 23:59 UTC displayed as 5:59 PM MST — cutting rental 6 hours short
  const endDate = `${pickupDate}T05:59:00+00:00`;
...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    await supabase.from("rental_access_codes").insert({
      ...
      lock_id: lockId,
      // Store in local time equivalent (midnight MST = 06:00 UTC, 11:59 PM MST = next day 05:59 UTC)
      start_time: `${booking.drop_off_date}T06:00:00+00:00`,
      end_time: `${booking.pickup_date}T05:59:59+00:00`,
      status: "active"
    });

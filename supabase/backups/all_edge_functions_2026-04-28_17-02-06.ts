...
// ----------------------------
// Function: igloohome-generate-pin
// ----------------------------
...
Deno.serve(async (req)=>{
  try {
    ...
    const bluetooth_id = Deno.env.get("IGLOOHOME_BLUETOOTH_ID") ?? "REDACTED_LOCK_ID";
...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    const oauthParams = new URLSearchParams({
      grant_type: "client_credentials",
      client_id: "REDACTED_IGLOOHOME_CLIENT_ID",
      client_secret: "REDACTED_IGLOOHOME_SECRET"
    });
    ...
    console.log(`[${timestamp}] [generate-igloohome-pin] Lock ID: REDACTED_LOCK_ID`);
    ...
    const igloohomeResponse = await fetch("https://api.igloohome.co/v2/locks/REDACTED_LOCK_ID/access-codes", {
      method: "POST",
      headers: {
        "Authorization": `Bearer ${accessToken}`,
        "Content-Type": "application/json"

...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    // Adjust service ID if delivery variant
    ...
    // Bookings (block full days)
    const { data: bookings, error: bookingsError } = await supabaseAdmin.from('bookings').select('drop_off_date, pickup_date').in('drop_off_date', dateRange).or(`pickup_date.in.(${dateRange.join(',')})`);
    if (bookingsError) throw bookingsError;
    const bookedDates = new Set(bookings.flatMap((b)=>[
        b.drop_off_date,
        b.pickup_date
      ].filter(Boolean)));
    // Unavailable dates
    const { data: unavailableDates, error: unavailableError } = await supabaseAdmin.from('unavailable_dates').select('date').eq('service_id', serviceIdForAvail).in('date', dateRange);
    if (unavailableError) throw unavailableError;
    const unavailableSet = new Set(unavailableDates.map((u)=>u.date));
    ...
    for (const dateStr of dateRange){
      ...
      // Block out fully booked or unavailable days
      if (rule && rule.is_available === false) {
        isAvailable = false;
      }
      if (bookedDates.has(dateStr) || unavailableSet.has(dateStr)) {
        isAvailable = false;
      }
      const deliverySlots = isAvailable && rule ? generateSlotsFromRange(rule.delivery_start_time, rule.delivery_end_time, 30, date, now) : [];
      const pickupSlots = isAvailable && rule ? generateSlotsFromRange(rule.pickup_start_time, rule.pickup_end_time, 30, date, now) : [];
      const returnSlots = isAvailable && rule ? generateSlotsFromRange(rule.return_start_time, rule.return_end_time, 30, date, now) : [];
      const hourlySlots = isAvailable && rule ? generateSlotsFromRange(rule.hourly_start_time, rule.hourly_end_time, 60, date, now) : [];

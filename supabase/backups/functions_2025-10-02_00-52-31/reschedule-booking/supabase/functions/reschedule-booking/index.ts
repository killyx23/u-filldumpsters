...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    const { error: noteError } = await supabaseAdmin.from('customer_notes').insert({
      ...
      source: 'System (Reschedule Request)',
      content: noteContent,
      author_type: 'system',
      is_read: false
    });
    ...
    // This part is a placeholder. You would use your email service (e.g., Resend) here.
    // Example: await sendAdminNotificationEmail(noteContent);

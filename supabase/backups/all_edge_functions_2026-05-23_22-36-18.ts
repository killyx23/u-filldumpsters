...
// --- File: create-first-admin/cors.ts ---
...
// --- File: create-first-admin/index.ts ---
...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    const { data: booking } = await supabase.from("bookings").select("id").eq("email", emailLower).order("created_at", {
      ascending: false
    }).limit(1).maybeSingle();
...
const generateEmailHTML = (booking, serviceDetails, insuranceAmount = 0)=>{
  ...
  if (serviceType === 'trailer_rental' || serviceName.toLowerCase().includes('dump loader') || serviceName.toLowerCase().includes('trailer')) {
    nextStepsHTML = `
      <li>Pick up the trailer at our location on ${formatDate(booking.drop_off_date)} at ${formatBookingTime(booking.drop_off_time_slot, {
      isSelfService: true,
      isReturnBy: false
    })}.</li>
      ...
      <li>Return the trailer by ${formatDate(booking.pickup_date)} at ${formatBookingTime(booking.pickup_time_slot, {
      isSelfService: true,
      isReturnBy: true
    })}.</li>
      ...
      <div style="margin-bottom: 25px;">
        ...
        <table style="width: 100%; border-collapse: collapse;">
          <tr>
            ...
            <td style="padding: 8px 0; color: #1f2937;">${formatDate(booking.drop_off_date)} at ${formatBookingTime(booking.drop_off_time_slot, {
    isSelfService: isTrailerSelfService(booking),
    isReturnBy: false
  })}</td>
          </tr>
          <tr>
            <td style="padding: 8px 0; color: #6b7280; font-weight: bold;">Pickup:</td>
            <td style="padding: 8px 0; color: #1f2937;">${formatDate(booking.pickup_date)} by ${formatBookingTime(booking.pickup_time_slot, {
    isSelfService: isTrailerSelfService(booking),
    isReturnBy: true
  })}</td>
          </tr>

...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    // paymentIntentId is appended by Stripe to the return_url as ?payment_intent=pi_xxx
    // BookingConfirmation reads it from the URL and passes it here
    ...
    // This makes the function safe to call multiple times (e.g. user refreshes
    ...
    // ----------------------------------------------------------------
    // Step 2: Retrieve PaymentIntent from Stripe to get charge_id
    //         and verify the payment actually succeeded before we
    ...
    if (paymentIntentId) {
      try {
        ...
        // Hard-stop if the payment hasn't actually succeeded
        ...
        // Stripe Customer — create or retrieve so we always have an ID
        ...
        if (customerEmail) {
          // Check if we already stored a Stripe customer id for this customer
          ...
          if (existingStripeId) {
            ...
            // Search Stripe for an existing customer with this email first
            const existing = await stripe.customers.list({
              email: customerEmail,
              limit: 1
            });
        ...
        // Non-fatal: log and continue — we still want the booking confirmed
        // if Stripe retrieval fails for a transient reason. Stripe IDs will
        // remain null and can be patched manually.
    ...
    //         The on_payment_info_insert_sync_customer trigger will
    //         automatically sync these IDs up to the customers table.
    // ----------------------------------------------------------------
    ...
    if (paymentInfoError) {
      // Log but don't throw — a stripe_payment_info failure should not
      // prevent the booking from being confirmed.
    ...
    if (addons.equipment?.length > 0) {
      // Use dbId if present (set by AddonsForm), otherwise fall back to
      // the name-based lookup that the old finalize-booking used.
    ...
    if (finalStatus === "pending_verification") {
      customEmailMessage = "Your booking is currently on hold. Essential verification information (license plate and/or driver's license photos) was not provided. Our team will manually review your booking. If we cannot complete verification, your booking may be subject to cancellation as per our rental agreement.";
    } else if (finalStatus === "pending_review") {
      customEmailMessage = "Your booking is currently on hold for manual review because the provided address could not be automatically verified. Our team will check the details and contact you if there are any issues servicing your location.";
    }

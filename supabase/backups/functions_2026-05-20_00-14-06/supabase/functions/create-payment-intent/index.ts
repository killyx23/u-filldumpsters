...
import { calculateTaxFromLineItems } from "../_shared/bookingTax.ts";
...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    const { data: booking, error: fetchError } = await supabase.from("bookings").select("id, total_price, subtotal_before_tax, tax_amount, tax_rate_used, status, plan, addons").eq("id", booking_id).single();
    ...
    let taxableSubtotal = Number(b.addons?.taxableSubtotal ?? 0);
    let nonTaxableSubtotal = Number(b.addons?.nonTaxableSubtotal ?? 0);
    ...
    const expectedTotalCents = Math.round((subtotalBeforeTax + taxAmount) * 100);
    if (subtotalBeforeTax > 0 && taxAmount >= 0 && Math.abs(amountInCents - expectedTotalCents) > 1) {
      console.warn(`[${timestamp}] [create-payment-intent] total_price (${b.total_price}) differs from subtotal+tax (${subtotalBeforeTax + taxAmount})`);
    }
    const metadata = {

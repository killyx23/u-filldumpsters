...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: verification, error: verificationError } = await supabase
      ...
      .select('email, is_verified, code_expires_at')
      .eq('email', normalizedEmail)
      .maybeSingle();

    if (verificationError) {
      return new Response(JSON.stringify({ success: false, error: 'Failed to validate verification' }), {
        status: 500,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
    ...
    if (customerError) {
      return new Response(JSON.stringify({ success: false, error: 'Failed to load customer profile' }), {
        status: 500,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }

...
async function generatePDFReceipt(booking, insuranceAmount = 0) {
  ...
  // ── Fees ──────────────────────────────────────────────────────────────
  const fees = [];
  if (insuranceAmount > 0) fees.push({
    name: 'Rental Insurance',
    amount: insuranceAmount
  });
  if (booking.addons?.drivewayProtection === 'accept') fees.push({
  ...
  y -= 14;
  drawText(`Tax (${storedTaxRate.toFixed(2)}%):`, col2X, y, {
    size: 10,
    color: gray
  });
...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    const { data: insuranceService } = await supabaseAdmin
      .from('services')
      .select('base_price')
    ...
    const pdfBytes = await generatePDFReceipt(booking, insuranceAmount);
